...
class CaseExporter:
    ...
    def export_to_xmind(self, test_cases: List[Dict[str, Any]], output_path: str) -> str:
        """
        导出测试用例到XMind - 嵌套层级结构
        ...
        try:
            ...
            for test_case in test_cases:
                ...
                expected_text = self._format_numbered_content(
                    "\n".join([str(e) for e in expected]) if isinstance(expected, list) else str(expected)
                )
                ...
                # 预期结果是操作步骤的子主题
                expected_node = self._create_node(expected_text, [priority_node], "expected")
                # 操作步骤是前置条件的子主题
                steps_node = self._create_node(steps_text, [expected_node], "steps")
                # 前置条件是测试标题的子主题
                precondition_node = self._create_node(precondition, [steps_node], "precondition")
                # 测试标题是测试点的子主题
                title_node = self._create_node(title, [precondition_node], "title")
                # 测试点是模块名称的子主题
                testpoint_node = self._create_node(test_point, [title_node], "testpoint")
                # 模块名称是用例编号的子主题
    ...
    def _create_flat_case_node(self, case_no, module, test_point, title, precondition, steps, expected, priority):
        """
        创建扁平化的用例节点
        层级：用例编号 -> 模块名称、测试点、测试标题、前置条件、操作步骤、预期结果、优先级
        """
    ...
    test_cases = [
        {
            ...
            "test_steps": ["1. 打开登录页面", "2. 输入用户名", "3. 输入密码", "4. 点击登录"],
