...
class ChromaVectorStore:
    ...
    def _validate_collections(self):
        ...
        try:
            # 尝试查询以验证hnsw索引是否正常
            self.case_collection.query(
                query_texts=["test"],
                n_results=1
            )
        except Exception as e:
            error_msg = str(e)
            if "hnsw" in error_msg.lower() or "nothing found on disk" in error_msg.lower():
                print(f"检测到hnsw索引损坏，正在重建...")
                self._rebuild_database()
            else:
                raise
    
    def add_requirement(self, requirement_id: str, content: str,
    ...
    def add_case(self, case_id: str, content: str,
                 metadata: Optional[Dict[str, Any]] = None):
        """
        添加历史用例到向量库
    ...
    def add_defect(self, defect_id: str, content: str,
                   metadata: Optional[Dict[str, Any]] = None):
        """
        添加缺陷到向量库
    ...
    def search_similar_requirements(self, query: str, top_k: int = 3) -> List[Dict[str, Any]]:
        """
        搜索相似需求
    ...
    def search_similar_defects(self, query: str, top_k: int = 3) -> List[Dict[str, Any]]:
        """
        搜索相似缺陷
...
class RAGEnhancer:
    ...
    def enhance_prompt(self, requirement_content: str, 
                       top_k_cases: int = 3,
                       top_k_defects: int = 2) -> str:
        """
    ...
    def build_few_shot_examples(self, requirement_content: str, 
